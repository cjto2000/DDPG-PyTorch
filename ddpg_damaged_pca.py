...
class DDPG:
    def __init__(self, env, memory, hidden_dim, include_loss=False):
        ...
        self.critic_net.load_state_dict(torch.load("models/history08/critic.pth", map_location=device))
    ...
    def train_one_episode(self, batch_size=32):
        ...
        while not is_done and n_steps < THRESHOLD_STEPS:
            ...
            self.memory.add_to_memory((S, A, S_prime, R, is_done, n_steps))
            # update the next state for next iteration
            S = S_prime
            R_total += R

            # Training on samples drawn from replay memory
            S_batch, A_batch, S_prime_batch, R_batch, is_done_batch, time_step_batch = self.memory.sample(batch_size)
            # cast into variables
        ...
        self.noise.reset()
        return critic_loss, actor_loss, R_total, n_steps, loss
